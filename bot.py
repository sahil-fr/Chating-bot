...
FIXED_REPLIES = {
    ...
    "can you speak in english": "Yes, I can speak english fluently",
...
def reply_user(message):
    ...
    for key, value in FIXED_REPLIES.items():
        if text.strip() == key:
            smart_reply(chat_id, value, message.message_id)
            return

    for word, reply in TRIGGERS.items():
        if word in text:
            smart_reply(chat_id, reply, message.message_id)
            return

    if should_reply:
        smart_reply(chat_id, random.choice(RANDOM_MESSAGES), message.message_id)
...
def welcome_new_members(message):
    ...
    for member in message.new_chat_members:
        ...
        smart_reply(chat_id, f"@{username} {random.choice(WELCOME_MESSAGES)}")
